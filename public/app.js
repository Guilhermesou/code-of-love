...
import { newDraft, occasions, calendarDays, closing, counterLabel, validate, mediaURL, reviewDraft } from './model.js';
import { readDraft, saveDraft } from './store.js';
import { mountExperience } from './experience.js';
import { buildGiftHTML } from './export.js';
import { cloudEnabled, getSession, onAuthStateChange, signInWithEmail, signOut, publishSurprise, fetchMyLatest } from './lib/cloud.js';
...
function accountWidget() {
  ...
  return `<form id="signin-form" class="account-form"><input type="email" id="signin-email" placeholder="seu@email.com" required><button class="text-button" type="submit" ${cloudBusy ? 'disabled' : ''}>Entrar</button></form>`;
}
function cloudSection() {
  ...
  const link = draft.cloudSlug ? `<p class="helper">Link atual: <a href="${esc(shareURL(draft.cloudSlug))}" target="_blank" rel="noopener noreferrer">${esc(shareURL(draft.cloudSlug))}</a></p>` : '';
  return `<div class="edit-card"><h2>Guardar e compartilhar na nuvem</h2><p class="helper">Envia uma cópia com as mídias para a nuvem, associada a ${esc(session.user.email)}.</p><div class="backup-actions"><button class="text-button" id="cloud-save" ${cloudBusy ? 'disabled' : ''}>Salvar rascunho na nuvem</button><button class="text-button" id="cloud-publish" ${cloudBusy ? 'disabled' : ''}>Gerar link para compartilhar</button><button class="text-button" id="cloud-restore" ${cloudBusy ? 'disabled' : ''}>Restaurar da nuvem</button></div>${link}</div>`;
}
async function runCloudAction(action, successMessage, isPublish = false) {
...
async function restoreFromCloud() {
  ...
  try {
    ...
    draft = { ...row.draft, cloudSlug: row.slug }; undoRemoval = null; persist(); notify('Rascunho restaurado da nuvem.');
  } catch { notify('Não foi possível restaurar da nuvem agora.'); }
  finally { cloudBusy = false; render(); }
}
const steps = ['Vocês', 'As datas', 'Lembranças', 'Voz & lugares', 'Cartas & final', 'Entregar'];
...
function persist() {
  ...
  const status = document.getElementById('save-state'); if (status) status.textContent = 'Salvando…';
  saveChain = saveChain.catch(() => {}).then(() => saveDraft(snapshot)).then(() => { storageError = false; const s = document.getElementById('save-state'); if (s) s.textContent = 'Rascunho salvo neste navegador'; }).catch(() => { storageError = true; const s = document.getElementById('save-state'); if (s) s.textContent = 'Não foi possível salvar'; notify('Não foi possível salvar neste navegador. Baixe uma cópia do rascunho para guardar seu trabalho.'); });
}
...
function panel() {
  if (step === 0) return `<span class="eyebrow">01 / os protagonistas</span><h1>Uma história.<br><em>Vocês dois.</em></h1><p class="intro">Comece pelos nomes. O resto a gente prepara, uma lembrança de cada vez.</p><div class="two-fields">${input('recipient', 'Para quem é a surpresa?', 'Nome ou apelido da pessoa')}${input('sender', 'E o seu nome?', 'Como você assina seus bilhetes')}</div>${select('relationship', 'Hoje, vocês estão…', { casados: 'Casados', namorando: 'Namorando', noivos: 'Noivos', juntos: 'Construindo nossa história' })}<div class="editor-note"><span>♡</span><p>O jeito de vocês importa. Os textos vão respeitar a fase atual do casal, mesmo ao celebrar uma data do começo.</p></div><fieldset class="palette"><legend>O tom da surpresa</legend>${['#c4685f', '#b0524f', '#c98a6b', '#a85c6b'].map((c, i) => `<label style="--swatch:${c}"><input type="radio" name="accent" value="${c}" ${draft.accent === c ? 'checked' : ''}><span class="swatch"></span><span class="sr-only">${['Rosa queimado', 'Vinho', 'Cobre', 'Malva'][i]}</span></label>`).join('')}<span class="palette-name">Romance cinematográfico</span></fieldset>`;
...
function render() {
  ...
  if (checkMusic) checkMusic.onclick = () => {
    ...
    container.innerHTML = info.type === 'spotify' ? `<iframe class="spotify-player" title="Conferir música no Spotify" src="${esc(info.embed)}" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture"></iframe><p class="helper">A reprodução disponível é definida pelo Spotify.</p><a class="text-button" href="${esc(info.url)}" target="_blank" rel="noopener noreferrer">Ouvir no Spotify ↗</a>` : info.type === 'youtube' ? `<p class="helper">Link reconhecido. A surpresa terá este convite para ouvir, sem vídeo na página.</p><a class="text-button" href="${esc(info.url)}" target="_blank" rel="noopener noreferrer">Ouvir no YouTube ↗</a>` : info.type === 'file' ? `<audio controls src="${esc(info.url)}" aria-label="Conferir música"></audio>` : '<p class="helper" role="status">Adicione um arquivo de áudio ou um link completo válido.</p>';
...
async function restoreDraft(e) {
  ...
  try {
    ...
    const value = JSON.parse(await file.text()), base = newDraft();
    if (value.version !== 1 || !value.dates || !['moments', 'letters', 'places'].every(k => Array.isArray(value[k]))) throw Error();
    for (const key of Object.keys(base)) if (typeof base[key] === 'string' && typeof value[key] !== 'string') throw Error();
    if (!Object.hasOwn(occasions, value.occasion) || !/^#[a-f0-9]{6}$/i.test(value.accent)) throw Error();
    for (const key of Object.keys(base.dates)) if (typeof value.dates[key] !== 'string') throw Error();
    for (const [key, fields] of Object.entries({ moments: ['title', 'date', 'text', 'photo'], letters: ['title', 'text'], places: ['name', 'lat', 'lng', 'date', 'memory', 'photo'] })) if (value[key].length > 100 || !value[key].every(item => item && fields.every(f => typeof item[f] === 'string'))) throw Error();
    if (!confirm('Restaurar esta cópia vai substituir o rascunho atual neste navegador. Deseja continuar?')) return;
    draft = Object.fromEntries(Object.keys(base).map(key => [key, value[key]])); undoRemoval = null; persist(); render(); notify('Rascunho restaurado.');
