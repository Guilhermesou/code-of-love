...
function panel() {
  if (step === 0) return `<span class="eyebrow">01 / os protagonistas</span><h1>Uma história.<br><em>Vocês dois.</em></h1><p class="intro">Comece pelos nomes. O resto a gente prepara, uma lembrança de cada vez.</p><div class="two-fields">${input('recipient', 'Para quem é a surpresa?', 'Nome ou apelido da pessoa')}${input('sender', 'E o seu nome?', 'Como você assina seus bilhetes')}</div>${select('relationship', 'Hoje, vocês estão…', { casados: 'Casados', namorando: 'Namorando', noivos: 'Noivos', juntos: 'Construindo nossa história' })}<div class="editor-note"><span>♡</span><p>O jeito de vocês importa. Os textos vão respeitar a fase atual do casal, mesmo ao celebrar uma data do começo.</p></div><fieldset class="palette"><legend>O tom da surpresa</legend>${['#c4685f', '#b0524f', '#c98a6b', '#a85c6b', '#6f7a52'].map((c, i) => `<label style="--swatch:${c}"><input type="radio" name="accent" value="${c}" ${draft.accent === c ? 'checked' : ''}><span class="swatch"></span><span class="sr-only">${['Rosa queimado', 'Vinho', 'Cobre', 'Malva', 'Verde musgo'][i]}</span></label>`).join('')}<span class="palette-name">Romance cinematográfico</span></fieldset>`;
