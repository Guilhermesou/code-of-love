...
export function buildGiftHTML(draft, css) {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Uma surpresa para ${title}</title><style>${css}</style></head><body><div id="app"></div><script>const getGameContent = ${getGameContent.toString()}; const installGames = ${installGames.toString()}; const parseMusic = ${parseMusic.toString()}; const installOpening = ${installOpening.toString()}; (${mountExperience.toString()})(${data});<\/script></body></html>`;
}
